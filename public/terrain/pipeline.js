import { fingerprintTerrainGenerationControls, } from './controls';
...
export function computeGenerationDirty(prev, next) {
    const meshChanged = prev.spacing !== next.spacing || prev.seed !== next.seed;
    const waterChanged = prev.waterLevel !== next.waterLevel ||
    ...
    const elevationChanged = prev.landRelief !== next.landRelief ||
    ...
    const riversChanged = prev.riverDensity !== next.riverDensity ||
        prev.riverBranchChance !== next.riverBranchChance ||
        prev.riverClimbChance !== next.riverClimbChance;
    const provincesChanged = prev.provinceCount !== next.provinceCount ||
...
export function toTerrainGenerationState(cache) {
