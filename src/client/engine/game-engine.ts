...
export class GameEngine {
	...
	private selector : Graphics | null = null;
	...
	async init(field: HTMLElement | null): Promise<void> {
		...
		const selector_graphics = new Graphics();
		selector_graphics.poly([-5, -10, 5, -10, 0,0], true);
		selector_graphics.fill({ color: '#aaaaaa'});
		selector_graphics.stroke({ width: 1, color: 0x000000, alpha: 0.9 });
		selector_graphics.eventMode = 'none';

		// ✅ center pivot based on its bounds
		const b = selector_graphics.getLocalBounds();
		selector_graphics.pivot.set(b.x + b.width / 2, b.y + b.height / 2);

		
		hudLayer.addChild(selector_graphics);
		
		this.selector = selector_graphics;
	...
	renderPlayers(players: PlayerState[]): void {
		...
		uiLayer.removeChildren(1); // 1 is selector
		players.forEach((player, index) => {
			...
			const row = new Text((player.emoji || '\ud83e\udd34') + ':', style);
			row.x = 0;
			row.y = index * 36;
